...
using System.Collections;  // Added for Unity's IEnumerator
using System.IO;
...
public class SingleToggleParentSetter : MonoBehaviour
{
   ...
   // public GameObject notificationPrefab;
    ...
    private void FullSceneScreenshot(int index)
    {
        ...
        string dirPath = @"C:\Users\anoth\AppData\LocalLow\DefaultCompany\Debug\user_study\screenshots\Baseline Screenshot\Laurie";
    ...
    {
        StartCoroutine(DeactivateNotificationWithDelay());
    ...
    */
    ...
    /*
    private IEnumerator DeactivateNotificationWithDelay()
    {
    ...
    */
    ...
    private void ToggleParent(int index)
    {
        if (activeToggleIndex == index)
        {
            ...
            */
            ...
            // Start the coroutine to set the toggle active/inactive after a delay
            StartCoroutine(SetToggleActiveAfterDelay(activeToggleIndex, false)); // Hide the current toggle button


            if (activeToggleIndex < toggles.Length)
            {
                // Schedule the next toggle button to be activated after a delay
                StartCoroutine(SetToggleActiveAfterDelay(activeToggleIndex + 1, true)); // Show the next toggle button as untoggled
            }
    ...
    private IEnumerator PerformOperationsAfterScreenshot(int index)
    {
        ...
        if (isToggleOn[index])
        {
            ...
            yield return new WaitForSeconds(1f);  // Added delay before setting toggle active/inactive
    ...
    private IEnumerator ActivateNotificationAfterDelay(int minDelay, int maxDelay)
    {
        ...
        // Set the position of the notificationPrefab relative to the camera
        notificationPrefab.transform.position = cameraPosition + new Vector3(-0.255f, -0.066f, 1.03f);
    ...
    */
    ...
    private void GoBack()
    {
        if (activeToggleIndex > 0)
        {
            ...
            StartCoroutine(SetToggleActiveAfterDelay(activeToggleIndex, false));

            activeToggleIndex--;

            StartCoroutine(SetToggleActiveAfterDelay(activeToggleIndex, true));
    ...
    private IEnumerator SetToggleActiveAfterDelay(int index, bool value)
    {
        if (value)
        {
            toggles[index].gameObject.SetActive(true);      // First set the toggle visible
            toggles[index].enabled = false;                 // Disable the toggle button
            yield return new WaitForSeconds(1f);            // Wait for 1 second
            toggles[index].enabled = true;                  // Enable the toggle button after the delay
        }
        else
        {
            yield return new WaitForSeconds(1f);
            toggles[index].gameObject.SetActive(false);     // Hide the toggle
        }
    }
    ...
    private void CheckForKeyboardInput()
    {
        ...
        //if (Input.GetKeyDown(KeyCode.N)) OnNotificationCloseButtonClicked();
    }
